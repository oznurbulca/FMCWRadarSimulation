...
for k=1:N
    ...
    object_parameters(k,1)=input('What is the range of the target (meters):'); 
    %object_parameters(k,2)=rand()*max_radial_velocity;
    object_parameters(k,2)=0; %velocity

    object_parameters(k,3)=deg2rad(input('What is the angle of the target (degrees):')); %angle 
