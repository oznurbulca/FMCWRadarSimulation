...
for k=1:N
    ...
    object_parameters(k,2)=-5; %velocity
    object_parameters(k,3)=0; %angle 
...
for object_num=1:N
    ...
    angle_object_i=object_parameters(object_num,3);
    ...
    for v =1:V_num
        % for tx = 1:num_tx
        %     for rx = 1:num_rx
                % steering_vector = exp(-1j * 2 * pi * (rx_spacing / lambda) * cosd(angle_object_i));
                R_rf_i=0;
                %R_rf=R_rf+A_i*exp(-1i*2*pi*fd)*p_t; %excluded fc
                % R_rf(v,:)=R_rf(v,:)+ steering_vector*A_i*exp(-1i*2*pi*fd*time_axis_Srf).*[zeros(1,Ti_n) Srf_n(1:end-Ti_n)]; %delayed signal
                R_rf(v,:)=R_rf(v,:)+ A_i*exp(-1i*2*pi*fd*time_axis_Srf).*[zeros(1,Ti_n) Srf_n(1:end-Ti_n)]; %delayed signal
...
for l = 1:V_num
    ...
    range_spectrum=fftshift(fft(IF_matrix_AD,[],2));
...
angular_spectrum = fftshift(fft(RDMaps, [], 1), 1);


cube = abs(angular_spectrum);

velocity_bin = K;  % Try different velocity bins, e.g., 1, K/2, etc.
